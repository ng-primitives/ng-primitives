...
export class NgpInputOtp {
  ...
  /**
   * The placeholder character to display in empty slots.
   */
  ...
  /**
   * Store the registered slot directives.
   * @internal
   */
  private readonly registeredSlots = new Map<number, any>();
  ...
  /**
   * Register a slot directive.
   * @param slot The slot directive to register.
   * @returns The assigned index for this slot.
   * @internal
   */
  registerSlot(slot: any): number {
    const index = this.registeredSlots.size;
    this.registeredSlots.set(index, slot);
    return index;
  }

  /**
   * Unregister a slot directive.
   * @param index The index of the slot to unregister.
   * @internal
   */
  unregisterSlot(index: number): void {
    this.registeredSlots.delete(index);
  }
}
