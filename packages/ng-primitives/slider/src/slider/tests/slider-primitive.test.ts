...
describe('NgpSlider', () => {
  ...
  describe('keyboard navigation (RTL)', () => {
    const imports = [NgpSlider, NgpSliderTrack, NgpSliderRange, NgpSliderThumb];

    async function renderRtl(value = 50) {
      const valueChange = vi.fn();
      const view = await render(
        createTemplate(
          `dir="rtl" [ngpSliderValue]="value" [ngpSliderMin]="0" [ngpSliderMax]="100" [ngpSliderStep]="10"`,
        ),
        { imports, componentProperties: { value, valueChange } },
      );
      return { valueChange, thumb: view.getByTestId('thumb') };
    }

    it('should increase the value on ArrowLeft in RTL', async () => {
      const { thumb, valueChange } = await renderRtl(50);
      fireEvent.keyDown(thumb, { key: 'ArrowLeft' });
      expect(valueChange).toHaveBeenCalledWith(60);
    });
    ...
    it('should move to the maximum on End regardless of RTL', async () => {
      const { thumb, valueChange } = await renderRtl(50);
  ...
  it('should respect step value when setting value via pointer', async () => {
    ...
    // Create and dispatch a MouseEvent at 73% of the track (should snap to 70)
    const pointerEvent = new MouseEvent('pointerdown', {
      bubbles: true,
      cancelable: true,
      clientX: 73,
