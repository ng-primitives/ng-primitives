...
describe('Navigation Menu', () => {
  ...
  describe('Focus Management', () => {
    it('should close on Escape and restore focus to trigger', async () => {
      ...
      await waitFor(() => {
        content = document.querySelector('[data-testid="content-products"]');
        expect(content).toBeInTheDocument();
        expect(trigger).toHaveAttribute('data-open');
      });

      // Dispatch Escape on the trigger instead of content, since the overlay's
      // document-level capture listener handles closing regardless of target
      fireEvent.keyDown(trigger, { key: 'Escape' });
      fixture.detectChanges();
