...
@Directive({
  ...
  host: {
    role: 'checkbox',
    '[attr.aria-checked]': 'indeterminate() ? "mixed" : formState.value()',
    '[attr.data-checked]': 'formState.value() ? "" : null',
    '[attr.data-indeterminate]': 'indeterminate() ? "" : null',
    '[attr.aria-disabled]': 'formState.disabled()',
    '[tabindex]': 'formState.disabled() ? -1 : 0',
  },
})
export class NgpCheckbox implements NgpCanDisable {
   ...
   */
  protected readonly formState = controlState({
    value: this.checked,
  ...
  toggle(event?: Event): void {
    ...
    this.formState.setValue(this.indeterminate() ? true : !this.formState.value());
