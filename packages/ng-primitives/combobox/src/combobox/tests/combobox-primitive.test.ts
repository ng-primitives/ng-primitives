...
function removeLingeringDropdown() {
  // the dropdown should be removed from the DOM after each test to avoid
  // interference with other tests - it may linger due to waiting for animations
...
describe('NgpCombobox', () => {
  ...
  describe('roles & attributes', () => {
    ...
    it('should have appropriate ARIA attributes on the input', async () => {
      ...
      // aria-controls should reference the open listbox
    ...
    it('should render the dropdown with role="listbox"', async () => {
      await render(TestComponent);
      const button = screen.getByTestId('combobox-button');
      await userEvent.click(button);
    ...
    it('should set aria-selected on the selected option', async () => {
      ...
      expect(screen.getByText('Apple')).not.toHaveAttribute('aria-selected');
    });
  });
