  ...
  template: `
    ...
    <ng-template #menu>
      <div ngpMenu data-testid="menu">
        ...
        <div ngpMenuItemDisabled ngpMenuItem data-testid="item-disabled">Disabled Item</div>
      </div>
    </ng-template>
  `,
  imports: [NgpMenuTrigger, NgpMenu, NgpMenuItem],
})
class TestMenuDisabledDivItemComponent {}
...
describe('NgpMenuItem disabled state', () => {
  ...
  it('should NOT close the menu when a disabled item is clicked (item is inert)', async () => {
    // A non-button host is used so the native `disabled` attribute cannot mask
    // the behaviour - the directive itself must ignore the click while disabled.
    const { fixture } = await render(TestMenuDisabledDivItemComponent);
    const trigger = fixture.debugElement.nativeElement.querySelector('[data-testid="trigger"]');
    ...
    // The menu must stay open - a disabled item performs no action.
    expect(trigger).toHaveAttribute('data-open');
    expect(document.querySelector('[data-testid="menu"]')).toBeInTheDocument();
  });
});
