...
describe('NgpFormControl', () => {
  ...
  describe('aria-invalid', () => {
    it('should not expose aria-invalid without a tracked control', async () => {
      const { getByTestId } = await render(`<input ngpFormControl data-testid="control" />`, {
        imports: [NgpFormControl],
      });
      expect(getByTestId('control')).not.toHaveAttribute('aria-invalid');
    });

    it('should not expose aria-invalid when the control is valid', async () => {
      const { getByTestId } = await render(
        `<input ngpFormControl data-testid="control" [formControl]="formControl" />`,
        {
          imports: [NgpFormControl, ReactiveFormsModule],
