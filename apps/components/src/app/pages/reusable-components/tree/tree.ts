...
@Component({
  ...
  template: `
    <ul
      ...
      [ngpTreeDefaultExpandedKeys]="defaultExpandedKeys()"
      ngpTree
    >
...
export class Tree {
  ...
  /** The node values expanded on first render. */
  readonly defaultExpandedKeys = input<ReadonlySet<string>>(new Set());

  protected readonly children = (node: TreeNode) => node.children;
  protected readonly itemValue = (node: TreeNode) => node.id;
  protected readonly itemLabel = (node: TreeNode) => node.name;
}
