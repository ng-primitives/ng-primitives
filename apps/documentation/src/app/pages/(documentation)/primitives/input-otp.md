...
A directive that represents an individual character slot. The directive automatically sets the slot's text content to either the character at that position or the placeholder character. Multiple slot elements must be created to match the `maxLength` of the input.
