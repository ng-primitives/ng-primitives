...
- <kbd>Tab</kbd> - Moves focus to the checked radio button, or the first enabled radio button when none is checked or the checked one is disabled. Clearing the value leaves the tab stop where it was, so focus returns to the radio button that was last checked.
